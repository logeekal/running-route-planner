...
const SideNav = forwardRef<HTMLDivElement, HTMLProps<HTMLDivElement>>(
  (props, ref) => {
    ...
    return (
      <aside
        className={`sidenav__container bg-primary ${
          className ? className : ""
        }
        ${collapsed ? "collapsed" : ""} absolute md:relative z-10 h-screen
        `}
        ref={ref}
        {...restProps}
      >
        <div className="relative w-full h-full">
          <div
            className="absolute right-0 z-10 w-8 h-8 cursor-pointer sidenav__switch top-2 bg-accent md:hidden"
            style={{
